...
CREATE OR REPLACE PACKAGE BODY JOB_PKG AS
    ...
    BEGIN
        IF p_max_salary < p_min_salary THEN
            RAISE_APPLICATION_ERROR(-20001, 'Maximum salary must be greater than or equal to minimum salary.');
        END IF;
    END VALIDATE_SALARY;
...
BEGIN
    JOB_PKG.ADD_JOB('IT_SYSAN', 'SYSTEMS ANALYST', 10000, 5000);
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
END;
...
BEGIN
    JOB_PKG.ADD_JOB('IT_DBADM', 'DATABASE ADMINISTRATOR', 5000, 10000);
    DBMS_OUTPUT.PUT_LINE('Job IT_DBADM added successfully.');
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
END;
...
BEGIN
    -- Update job IT_DBADM's title
    JOB_PKG.UPD_JOB('IT_DBADM', 'DATABASE ADMINISTRATOR');
    DBMS_OUTPUT.PUT_LINE('Job title updated successfully.');
